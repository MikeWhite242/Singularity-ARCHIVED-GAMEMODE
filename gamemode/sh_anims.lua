singularity.Anim = singularity.Anim or {}
singularity.Anim.citizen_male = {
	normal = {
...
singularity.Anim.citizen_female = {
	normal = {
		[ACT_MP_STAND_IDLE] = {ACT_IDLE, ACT_IDLE_MANNEDGUN},
		[ACT_MP_CROUCH_IDLE] = {ACT_COVER_LOW, ACT_COVER_LOW},
	...
	glide = ACT_GLIDE,
	vehicle = singularity.Anim.citizen_male.vehicle
}
singularity.Anim.metrocop = {
	normal = {
...
function singularity.Anim.SetModelClass(model, class)
	if not singularity.Anim[class] then
...
function singularity.Anim.GetModelClass(model)
...
singularity.Anim.SetModelClass("models/police.mdl", "metrocop")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/hdpolice.mdl", "metrocop")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/civil_medic.mdl", "metrocop")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/hl2beta_police.mdl", "metrocop")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/retrocop.mdl", "metrocop")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/elite_police.mdl", "metrocop")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/policetrench.mdl", "metrocop")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/hl2concept.mdl", "metrocop")
singularity.Anim.SetModelClass("models/combine_super_soldier.mdl", "overwatch")
singularity.Anim.SetModelClass("models/combine_soldier_prisonGuard.mdl", "overwatch")
singularity.Anim.SetModelClass("models/combine_soldier.mdl", "overwatch")
singularity.Anim.SetModelClass("models/vortigaunt.mdl", "vort")
singularity.Anim.SetModelClass("models/vortigaunt_blue.mdl", "vort")
singularity.Anim.SetModelClass("models/vortigaunt_doctor.mdl", "vort")
singularity.Anim.SetModelClass("models/vortigaunt_slave.mdl", "vort")
singularity.Anim.SetModelClass("models/vortiblue1.mdl", "vort")
singularity.Anim.SetModelClass("models/dpfilms/metropolice/police_fragger.mdl", "metrocop")
--landis.Anim.SetModelClass("models/breen.mdl", "player")
--landis.Anim.SetModelClass("models/nicrobe/gallahan.mdl", "player")
...
do
	...
	function meta:ForceSequence(sequence, callback, time, noFreeze)
		...
		if sequence and sequence > 0 then
			...
			self.singularitySeqCallback = callback
			self.singularityForceSeq = sequence
			...
			if time > 0 then
				timer.Create("singularitySeq"..self:EntIndex(), time, 1, function()
	...
	function meta:leaveSequence()
		...
		self.singularityForceSeq = nil

		if self.singularitySeqCallback then
			self:singularitySeqCallback()
		end
	end

	if SERVER then
		util.AddNetworkString("singularitySeqSet")
	...
	if CLIENT then
		net.Receive("singularitySeqSet", function()
			...
			if IsValid(ent) then
				if reset then
					ent.singularityForceSeq = nil
					return
				end

				ent:SetCycle(0)
				ent:SetPlaybackRate(1)
				ent.singularityForceSeq = sequence
			end
		end)
	end
end
...
local getModelClass = singularity.Anim.GetModelClass
...
function GM:TranslateActivity(ply, act)
	...
	if class == "player" then
		if IsValid(weapon) and not ply.IsWeaponRaised(ply) and ply.OnGround(ply) then
			...
			local animTree = singularity.Anim.player[holdType]
	...
	local animTree = singularity.Anim[class]
...
function GM:CalcMainActivity(ply, velocity)
	...
	return seqIdeal, ply.singularityForceSeq or oldSeqOverride or ply.CalcSeqOverride
end

function GM:DoAnimationEvent(ply, event, data)
	local model = ply:GetModel():lower()
	local class = singularity.Anim.GetModelClass(model)
	...
	if class == "player" then
		...
		if IsValid(weapon) then
			...
			local animation = singularity.Anim[class][holdType]
