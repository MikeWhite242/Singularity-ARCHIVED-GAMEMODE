...
singularity.Teams.ClassRef = singularity.Teams.ClassRef or {}
singularity.Teams.NameRef = singularity.Teams.NameRef or {}
teamID = 0

CLASS_EMPTY = 0

function singularity.Teams.Define(teamData)
    teamID = teamID + 1
    singularity.Teams.Data[teamID] = teamData
    singularity.Teams.NameRef[teamData.name] = teamID

    if teamData.classes then
    	singularity.Teams.Data[teamID].ClassRef = {}

    	for id,k in pairs(teamData.classes) do
    		singularity.Teams.Data[teamID].ClassRef[id] = k.name
    	end
    end

    if teamData.ranks then
    	singularity.Teams.Data[teamID].RankRef = {}

    	for id,k in pairs(teamData.ranks) do
    		singularity.Teams.Data[teamID].RankRef[id] = k.name
    	end
    end
...
function meta:CanBecomeTeam(teamID, notify)
	local teamData = singularity.Teams.Data[teamID]
...
function meta:CanBecomeTeamClass(classID, notify)
	local teamData = singularity.Teams.Data[self:Team()]
...
function meta:GetTeamClassName()
	if not singularity.Teams.Data[self:Team()] then return "" end

	local classRef = singularity.Teams.Data[self:Team()].ClassRef
...
function meta:GetTeamRankName()
	local rankData = singularity.Teams.Data[self:Team()].ranks
