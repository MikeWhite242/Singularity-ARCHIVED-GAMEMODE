...
function PANEL:SetMessage(text)
	...
	self.Paint = function ( self, w, h )
		...
		if CurTime() > self.removeTime and not self.fadingOut then
			...
			timer.Simple(0.5, function()
				self:Remove()
				table.RemoveByValue(singularity.Notifications, self)
			end)
		end
	...
	for v,k in ipairs(singularity.Notifications) do
	...
	table.ForceInsert(singularity.Notifications, self)
...
vgui.Register("singularityNotifyPanel",PANEL, "DPanel")