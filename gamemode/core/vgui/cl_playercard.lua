...
function PANEL:SetPlayer( ply )
	...
	for v,b in ipairs(singularity.Badges.Data) do
...
function PANEL:Init()
	...
	self.PaintOver = function( self, w, h )
		...
		draw.SimpleText(self.Player:Nick(), "SingularityFont23", 79, 35)
		local localTeam = self.Player:Team()
		draw.SimpleText(team.GetName( localTeam ), "SingularityFont27", 79, 67, team.GetColor( localTeam))
...
function PANEL:OnClose()
	singularity.Playercard = nil
end

vgui.Register("singularityPlayerCard", PANEL, "DFrame")
...
properties.Add("openPlayercard", {
	...
	Action = function(ply,self)
		if self:IsPlayer() then
			singularity.Playercard = singularity.Playercard or vgui.Create("singularityPlayerCard")
			Card:SetPlayer(self)
