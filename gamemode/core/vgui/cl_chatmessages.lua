...
singularity.DefineSetting("chatfontSize",{type="slider",default=18,min=8,max=48,dec=0,name="Chat Font Size",category="Chatbox"})

function PANEL:SetMessage( ... )

	local args = { ... }

	self.text    = "<font=SingularityFont" .. tostring(math.Clamp(math.floor(singularity.GetSetting("chatfontSize")), 8, 48) or 18) .. "><colour=255,255,255>"
	self.colors  = {}
	self.sender  = nil
	...
	local w   = singularity.chatbox.chatLog:GetWide() - 5
	self.message = markup.Parse( self.text, singularity.chatbox.chatLog:GetWide() )
...
function PANEL:DoClick()
	...
	if LocalPlayer():IsAdmin() then
		...
		local opt_b = playerMenu:AddOption( "[admin] Open Player Info", function() 
			local Card = vgui.Create("singularityPlayerCard")
