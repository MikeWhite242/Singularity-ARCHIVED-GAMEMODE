...
function singularity.RegisterChatCommand(className,struct)
	if not className or not struct then return end
	local makeStruct = table.Inherit(struct, baseStruct)
	-- shoud never fail
	if makeStruct then
		singularity.chat.commands[string.lower(className)] = makeStruct
		if SERVER then
			singularity.ConsoleMessage("Registered Chat Command: "..className)
		end
	end
end
...
hook.Add("PlayerSay", "chat-plugin-runner", function(ply,text)
	...
	if string.Left(text, 1) == "/" then
		...
		local validCommands = table.Copy(singularity.chat.commands)
